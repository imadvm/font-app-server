...
pub async fn check_duplicate(
    db_client: &tokio_postgres::Client, // Replace with your actual DB client type
    user_id: &uuid::Uuid,
    checksum: &str
) -> Result<Option<String>, Box<dyn std::error::Error>> {
    let query = "SELECT object_path FROM fonts WHERE user_id = $1 AND checksum = $2 LIMIT 1";

    match db_client.query_opt(query, &[&user_id, &checksum]).await? {
...
pub async fn get_metadata(
    ...
    let rows = db_client.query(query, &[&user_id]).await?;
